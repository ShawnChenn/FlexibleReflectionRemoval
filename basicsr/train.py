...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            datadir = '/data1/chenxiao/NAFNet/datasets/reflection-dataset'
            datadir_vocsyn = os.path.join(datadir, 'train/VOCdevkit/VOC2012/JPEGImages')
            datadir_real = os.path.join(datadir, 'train/real')
            datadir_nature = os.path.join(datadir, 'train/nature')
            
            # train_dataset = datasets.DSRDataset(datadir_syn, read_fns('/home/chenxiao/NAFNet/basicsr/data/VOC2012_224_train_png.txt'), enable_transforms=True)
            train_dataset = datasets.DSRDataset(datadir_vocsyn, enable_transforms=True)

            train_dataset_real = datasets.DSRTestDataset(datadir_real, enable_transforms=True, if_align=True, test_flag=False)
            train_dataset_nature = datasets.DSRTestDataset(datadir_nature, enable_transforms=True, if_align=True, test_flag=False)

            train_set = datasets.FusionDataset([train_dataset, train_dataset_real, train_dataset_nature], [0.6, 0.2, 0.2])
            
            train_sampler = EnlargedSampler(train_set, opt['world_size'],
                                            opt['rank'], dataset_enlarge_ratio)
            train_loader= datasets.DataLoader(train_set, batch_size=dataset_opt['batch_size_per_gpu'], shuffle=True,
            ...
            val_set = datasets.DSRTestDataset(os.path.join(datadir, f'test/real20_420'),
                                            fns=read_fns('/data1/chenxiao/NAFNet/basicsr/data/real_test.txt'), if_align=True, test_flag=True)
            val_loader = datasets.DataLoader(val_set, batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
            test_loaders.append(val_loader)

            # val_set = datasets.DSRTestDataset(os.path.join(datadir, f'test/Nature'), if_align=True, test_flag=True)
            # val_loader = datasets.DataLoader(val_set, batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
            # test_loaders.append(val_loader)
            
            test_set_solidobject = datasets.DSRTestDataset(os.path.join(datadir, 'test/SIR2/SolidObjectDataset'), if_align=True, test_flag=True)
    ...
    return train_loader, train_sampler, test_loaders, total_epochs, total_iters
...
if __name__ == '__main__':
    import os
    # img_path = '/data1/chenxiao/NAFNet/datasets/reflection-dataset/test/SIR2/PostcardDataset/allPairMask/003.png'
    ...
    # new_img.save('/data1/chenxiao/NAFNet/datasets/reflection-dataset/test/SIR2/PostcardDataset/allPairMask/003_1.png')
