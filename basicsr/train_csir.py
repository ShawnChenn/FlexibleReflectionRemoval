...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            datadir = '/home/chenxiao/eccv24_reflection_removal/datasets/reflection-dataset'
            # datadir_syn = os.path.join(datadir, 'train/VOCdevkit/VOC2012/PNGImages')
            datadir_syn = '/data0/coco/train2017'
            datadir_real = os.path.join(datadir, 'train/real')
            datadir_nature = os.path.join(datadir, 'train/nature')

            train_dataset = datasets.CoCoDataset(datadir_syn, read_fns('/home/chenxiao/NAFNet/basicsr/data/coco_train_png.txt'), enable_transforms=True)            
            ...
            train_dataset_real = datasets.DSRTestDataset(datadir_real, enable_transforms=True, if_align=True)
            train_dataset_nature = datasets.DSRTestDataset(datadir_nature, enable_transforms=True, if_align=True)

            train_set = datasets.FusionDataset([train_dataset, train_dataset_nature,
                                               train_dataset_real], [0.6, 0.2, 0.2])
            ...
            val_set = datasets.DSRTestDataset(os.path.join(datadir, f'test/Nature'), if_align=True)
            val_loader = datasets.DataLoader(val_set, batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
            test_loaders.append(val_loader)
            ...
            test_set_postcard = datasets.DSRTestDataset(os.path.join(datadir, 'test/SIR2/PostcardDataset'), if_align=True)
            ...
            test_set_wild = datasets.DSRTestDataset(os.path.join(datadir, 'test/SIR2/WildSceneDataset'), if_align=True)
...
# python basicsr/train.py -opt options/train/REDS/NAFNet-width64.yml