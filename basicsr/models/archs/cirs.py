...
from basicsr.models.archs.clip import build_model
import math
import hashlib
import os
import urllib
...
from basicsr.models.archs.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
def linear_layer(in_dim, out_dim, bias=False):
    return nn.Sequential(nn.Linear(in_dim, out_dim, bias),
                         nn.InstanceNorm1d(out_dim), nn.ReLU(True))
    # return nn.Sequential(nn.Linear(in_dim, out_dim, bias),
    #                       nn.ReLU(True))
...
class CRIS(nn.Module):
    def __init__(self):
        super().__init__()
        # Vision & Text Encoder
        clip_model = torch.jit.load("/home/chenxiao/NAFNet/RN50.pt",
                                    map_location="cpu").eval()
        self.backbone = build_model(clip_model.state_dict()).float()
        # Multi-Modal FPN
    ...
    def forward(self, img, word, mask=None):
        ...
        # b, 1, 104, 104
        # pred = torch.sigmoid(self.proj(fq, state))
...
if __name__ == "__main__":
    ...
    word = tokenize("a diagram")
    cmodel = CRIS()
    pred = cmodel(img, word)